...
This entropic framing resolves apparent paradoxes. Experiments have confirmed the entropic bound in standard erasure \citep{berut2012}, while feedback protocols have demonstrated work extraction from correlations \citep{toyabe2010}. These results are not violations of a ``$k_\mathrm{B}T\ln 2$ per bit'' rule---they are consequences of structure in the initial state or correlations with a measurement apparatus.
...
If the register $X$ is correlated with an auxiliary system $Y$, the erasure bound tightens further. Given access to $Y$, the minimal work to reset $X$ is \citep{sagawa2009} (see \citep{delrio2011} for quantum extensions)
\begin{equation}
